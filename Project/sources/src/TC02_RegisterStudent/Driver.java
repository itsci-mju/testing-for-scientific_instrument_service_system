...
public class Driver {
	...
	public String getIP(String vIP){
		...
		if (vIP.equals("VstuID")){
			vIP = StartUp.VstuID;
		}
		if (vIP.equals("VseDegree")){
			vIP = StartUp.VseDegree;
		}
		...
		if (vIP.equals("VadName")){
			vIP = StartUp.VadName;
		}
		if (vIP.equals("VadEmail")){
			vIP = StartUp.VadEmail;
		}
	...
	public void getData(int k) {
		...
		StartUp.VstuID = StartUp.xTDdata[k][12];
		StartUp.VseDegree = StartUp.xTDdata[k][13];
		StartUp.Vbranch = StartUp.xTDdata[k][14];
		StartUp.Vfaculty = StartUp.xTDdata[k][15];
		StartUp.Vuniversity = StartUp.xTDdata[k][16];
		StartUp.VadName = StartUp.xTDdata[k][17];
		StartUp.VadEmail = StartUp.xTDdata[k][18];
		StartUp.Vemail = StartUp.xTDdata[k][19];
		StartUp.Vphone = StartUp.xTDdata[k][20];
		StartUp.xPath = StartUp.xTDdata[k][26];
