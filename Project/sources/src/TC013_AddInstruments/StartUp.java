...
public class StartUp {
	...
	public static String vUrl, Vuser, Vpwd,Sci_ID,Sci_Name,Detail,Staff,Course,Status,Photo,
	...
	public void main() throws  Throwable {
		...
		String vKeyword, vIP1, vIP2, vIP3, vIP4, vIP5, vIP6, vIP7, vIP8, vIP9, vIP10, vIP11, vIP12, vIP13, vIP14, vIP15, vIP16
		       , vIP17, vIP18;
		String xlPath_tc = "c:/Users/saram/MiniProject65/Resultsummary/TC/TC013_AddInstruments.xlsx";
		String xlPath_ts = "c:/Users/saram/MiniProject65/Resultsummary/TS/TS013_AddInstruments.xlsx";
		String xlPath_td = "c:/Users/saram/MiniProject65/Resultsummary/TD/TD013_AddInstruments.xlsx";
	    String xlPath ="c:/Users/saram/MiniProject65/Excel/TC013_AddInstruments.xlsx";
		...
		for (int i = 1; i < xTCdata.length; i++) {
			if (xTCdata[i][4].equals("Y")) {
		 		...
		 		for (int k = 1; k < xTDdata.length ; k++) {
					...
					for (int j = 1; j <xTSdata.length; j++){
						if (xTCdata[i][1].equals(xTSdata[j][0])){	
	                		...
	                		if (vResult.equalsIgnoreCase("Pass")) {
	                			...
	                			String expected = xTDdata[k][38];
	                			if(vKeyword.contains("get_textAlert")) {
	                				if(vKeyword.contains("")) {
	                					xTDdata[k][39] = "Fail";
	                					xTDdata[k][40] = vResult;
	                					StartUp.xTDdata[k][41] = "Error : Not match expected result.";
                					}
	                				if(vResult.equals(expected)) {
	                					xTDdata[k][40] = vResult;
	                					xTDdata[k][39] = "Pass";
	                					StartUp.xTDdata[k][41] = StartUp.vError;
	                				}else
	                				if(expected.equals("เพิ่มเครื่องมือวิทยาศาสตร์สำเร็จ") && vResult.equals("บันทึกสำเร็จสำเร็จ !!!!")) {
	                					xTDdata[k][40] = vResult;
	                					xTDdata[k][39] = "Pass";
	                					StartUp.xTDdata[k][41] = StartUp.vError;
	                				}else {
					...
					System.out.println("Expected = " + StartUp.xTDdata[k][38]);
					System.out.println("Result = " + StartUp.xTDdata[k][39]);
					System.out.println("Actual = " + StartUp.xTDdata[k][40]);
						
			}
					if (xTDdata[k][39] == "Fail") {
						fails += 1;
					} else if (xTDdata[k][39] == "Pass") {
						passes += 1;
					}
		}	
		 		if (fails != 0) {
