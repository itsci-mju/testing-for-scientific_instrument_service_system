...
public class StartUp {
	...
	public static String vUrl, Vuser, Vpwd,Language,Client_name,Address,
	                     Report_Address,Fax,Billing_Address,SampleName,
	                     Code,Weight,Qty,TestItemid,Storage,Packing_style,
	                     Additional,Other_addit,Receive,Results,xPath;
	...
	public void main() throws  Throwable {
		...
		String xlPath_tc = "c:/Users/saram/MiniProject65/Resultsummary/TC/TC04_FillTestingAnalysisForm.xlsx";
		String xlPath_ts = "c:/Users/saram/MiniProject65/Resultsummary/TS/TS04_FillTestingAnalysisForm.xlsx";
		String xlPath_td = "c:/Users/saram/MiniProject65/Resultsummary/TD/TD04_FillTestingAnalysisForm.xlsx";
	    String xlPath ="c:/Users/saram/MiniProject65/Excel/TC04_FillTestingAnalysisForm.xlsx";
		...
		for (int i = 1; i < xTCdata.length; i++) {
			if (xTCdata[i][4].equals("Y")) {
		 		...
		 		for (int k = 1; k < xTDdata.length ; k++) {
					...
					for (int j = 1; j <xTSdata.length; j++){
						if (xTCdata[i][1].equals(xTSdata[j][0])){	
	                		...
	                		if (vResult.equalsIgnoreCase("Pass")) {
	                			...
	                			String expected = xTDdata[k][23];
	                			if(vKeyword.contains("get_text1")) {
	                				if(vKeyword.contains("")) {
	                					xTDdata[k][25] = vResult;
	                					xTDdata[k][24] = "Fail";
	                					StartUp.xTDdata[k][26] = "Error : Not match expected result.";
	                				}
	                				
	                				if(vResult.equals(expected)) {
	                					xTDdata[k][25] = vResult;
	                					xTDdata[k][24] = "Pass";
	                					StartUp.xTDdata[k][26] = StartUp.vError;
                					}
	                				
	                				if(expected.equals("กรอกแบบฟอร์มการขอใช้บริการวิเคราะห์และทดสอบสำเร็จ") && vResult.equals("บันทึกสำเร็จ !!!!")) {
	                					xTDdata[k][25] = vResult;
	                					xTDdata[k][24] = "Pass";
	                					StartUp.xTDdata[k][26] = StartUp.vError;
	                				}else {
					...
					System.out.println("Expected = " + StartUp.xTDdata[k][23]);
					System.out.println("Result = " + StartUp.xTDdata[k][24]);
					System.out.println("Actual = " + StartUp.xTDdata[k][25]);
